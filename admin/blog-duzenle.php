...
$getBlog = "SELECT * FROM blogs WHERE blog_id = '$blog_id'";
$blog = $conn->query($getBlog)->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $blog_image = isset($_POST['blog_image']) && !empty($_POST['blog_image']) ? 
                     $conn->real_escape_string($_POST['blog_image']) : 
                     $blog['blog_image'];
        ...
        if ($e->getCode() == 1062) {
            ...
            $error = "Blog yazısı güncellenirken bir hata oluştu: " . $e->getMessage();
        }
    }
}
            ...
            <form method="POST" action="" enctype="multipart/form-data" class="space-y-6">
                <div>
                    ...
                    <input type="text" id="blog_title" name="blog_title" required
                           value="<?php echo $blog['blog_title']; ?>"
                    ...
                    <select id="blog_category_id" name="blog_category_id" required
                        ...
                        <?php while ($category = $categories->fetch_assoc()): ?>
                            <option value="<?php echo $category['category_id']; ?>" 
                                    <?php echo $category['category_id'] == $blog['blog_category_id'] ? 'selected' : ''; ?>>
                ...
                <div>
                    ...
                    <div class="upload-area rounded-lg p-4 text-center cursor-pointer" onclick="document.getElementById('blog_image').click()">
                        ...
                        <div id="upload-placeholder" class="text-gray-400 <?php echo $blog['blog_image'] ? 'hidden' : ''; ?>">
                            <i class="fas fa-cloud-upload-alt text-4xl mb-2"></i>
                            <p>Resim yüklemek için tıklayın veya sürükleyin</p>
                        </div>
                        <img id="image-preview" class="image-preview mx-auto <?php echo $blog['blog_image'] ? '' : 'hidden'; ?>" 
                             src="../<?php echo $blog['blog_image']; ?>" alt="Önizleme">
                        <button type="button" id="remove-image" class="<?php echo $blog['blog_image'] ? '' : 'hidden'; ?> mt-2 text-red-500 hover:text-red-600" 
                                onclick="removeImage()">
                ...
                <div>
                    ...
                    <div id="editor"><?php echo $blog['blog_content']; ?></div>
                ...
                <div>
                    ...
                    <select id="blog_status" name="blog_status" required
                            class="w-full px-4 py-2 rounded text-white border border-gray-600 focus:border-(--primary) focus:outline-none bg-gray-700">
                        <option value="draft" <?php echo $blog['blog_status'] === 'draft' ? 'selected' : ''; ?>>Taslak</option>
                        <option value="published" <?php echo $blog['blog_status'] === 'published' ? 'selected' : ''; ?>>Yayınla</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="blog_meta_title" name="blog_meta_title"
                           value="<?php echo $blog['blog_meta_title']; ?>"
                           class="w-full px-4 py-2 rounded text-white border border-gray-600 focus:border-(--primary) focus:outline-none">
                </div>
                ...
                <div>
                    ...
                    <textarea id="blog_meta_description" name="blog_meta_description" rows="3"
                              class="w-full px-4 py-2 rounded text-white border border-gray-600 focus:border-(--primary) focus:outline-none"><?php echo $blog['blog_meta_description']; ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="blog_meta_keywords" name="blog_meta_keywords"
                           value="<?php echo $blog['blog_meta_keywords']; ?>"
                           class="w-full px-4 py-2 rounded text-white border border-gray-600 focus:border-(--primary) focus:outline-none"
                           placeholder="Virgül ile ayırarak yazın">
                </div>

                <div class="flex justify-between items-center ">
                    <a href="bloglar.php" class="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 transition-colors">
